		...
		<>
			<header className="sticky top-0 z-50 w-full bg-l0">
				{bannerVisible && (
					<div className="relative flex h-9 w-full items-center justify-center gap-2 bg-l3 px-4 text-xs text-high-emphasis">
						...
						<span>
							...
							<span className="hidden sm:inline">
								{" "}
								is live - real matching engine, real order book.
							</span>
				...
				<div className="relative flex h-14 w-full items-center px-4">
					<Link to="/markets" className="text-lg font-semibold tracking-tight shrink-0">
						PaperDrill
			...
			{mobileOpen && (
				<>
					...
					/>
					<div
						className="fixed left-0 right-0 bottom-0 bg-l0 border-t border-border/40 md:hidden z-50 overflow-y-auto"
						style={{ top: bannerVisible ? "calc(3.5rem + 2.25rem)" : "3.5rem" }}
					>
						<nav className="flex flex-col gap-1 p-4 text-sm">
							<Link
								to="/"
								className="flex items-center h-11 px-3 rounded-lg text-medium-emphasis hover:text-high-emphasis hover:bg-l2 transition-colors"
								onClick={() => setMobileOpen(false)}
							>
								Home
							</Link>
							...
							<div className="flex items-center justify-between h-11 px-3 mt-1 border-t border-border/40">
							...
							<div className="flex flex-col gap-1 pt-2 border-t border-border/40 mt-1">
								{user ? (
									<div className="flex items-center gap-3 px-3 py-2">
										...
										>
											<div className="flex h-8 w-8 items-center justify-center rounded-full bg-primary/10 text-sm font-bold uppercase text-primary">
												{user.name[0]}
											</div>
